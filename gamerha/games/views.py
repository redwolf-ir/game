...
def api_view(request):
    if 'game_name_input' in request.POST:
        ...
        response = requests.get(
            'https://api.rawg.io/api/games?page_size=5&search={}'.format(game_name_safe))
        data = response.json()
        temp_list = []
        game_informations = []
        ...
        response = requests.get(
            'https://api.rawg.io/api/games/{}'.format(game_slug))
        data = response.json()
        platforms = []
        genres = []
        developers = []
        ...
        for i in range(0, len(data['metacritic_platforms'])):
            platforms.append(data['metacritic_platforms']
                             [i]['platform']['slug'])
        ...
        return render(request, 'add_new_game.html', context)
...
def add_new_game_view(request):
    if request.method == "POST":
        ...
        request.session['game_under_edit'] = context
...
class GameCreate(LoginRequiredMixin, CreateView):
    ...
    model = GAME
    fields = ["title", "release_date", "box_art", "slug", "genre",
              "developer", "publisher", "platforms", "Age_rating"]
    template_name = "add_new_game.html"

    def get_initial(self):
        if (self.request.session['game_under_edit']):
            game_under_edit = self.request.session['game_under_edit'],
        else:
            game_under_edit = 'nothing'
        return {
            'game_under_edit': game_under_edit,
        }
